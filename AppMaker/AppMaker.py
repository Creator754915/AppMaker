...
def about():
    abt = Tk()
    abt.title("About - AppMaker")
    abt.geometry("450x550")
    abt.resizable(False, False)

    Label(abt, text="About AppMaker", font=("Arial", 18)).pack(pady=2, side=TOP)

    Label(abt, text='''
AppMaker Version => Alpha 2.5
Python Version => 3.10

Description => AppMaker est un logiciel puissant conçu 
pour simplifier le processus de création d'applications 
en utilisant des interfaces visuelles, 
le tout intégré dans un environnement convivial. 
Que vous soyez un débutant ou un développeur expérimenté, 
AppMaker vous permet de concrétiser vos idées d'application 
sans avoir à écrire une seule ligne de code.
    ''', font=("Arial", 12)).place(relx=0.5, rely=0.5, anchor=CENTER)

    Label(abt, text="Copyright 2024 c", font=("Arial", 8)).pack(side=BOTTOM)
...
filemenu.add_command(label="Open", command=import_file)
